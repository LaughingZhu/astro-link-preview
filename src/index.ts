...
const integration = (options: Options = {}): AstroIntegration => {
  const { logStats = true, proxy, previewImageFormat = 'jpg', enableOnMobile = false } =
    options;
  ...
  return {
    ...
    hooks: {
      ...
      'astro:build:done': async ({ dir, pages }) => {
        ...
        const buildFormat =
          astroConfig.build.format === 'file' || astroConfig.build.format === 'directory'
...
export default integration; // 使用 export default 导出